\documentclass[
% a4paper, fontsize=10pt, BCOR=5mm, % reviewed version
  paper=6.69in:9.61in, fontsize=10pt, BCOR=9mm, DIV=15,
  pagesize=pdftex,
  headings=openright,
  headsepline,
  chapterprefix,
  appendixprefix,
  draft=false,
  listof=totoc,
  listof=leveldown
]{scrbook}

\usepackage[utf8]{inputenc}
\usepackage{graphicx}
\usepackage{csquotes}
\usepackage{booktabs}
\usepackage{listings}
\usepackage[magyar,ngerman,english]{babel}
\usepackage[hyphens]{url}
\PassOptionsToPackage{hyphens}{url}
\usepackage[hyperindex,breaklinks]{hyperref}
\usepackage{bookmark}
\usepackage{breakurl}
% textrightarrow U+2192
\usepackage{textcomp}
%\usepackage{enumitem}
%\setlist{nolistsep}
% algorithm
\usepackage{amsmath}
\usepackage{algorithm2e}

% for table positions -- https://tex.stackexchange.com/questions/9485/how-to-fix-table-position
\usepackage{float}


% for rotate tables
\usepackage{adjustbox}
\usepackage{pdflscape}
\usepackage{afterpage}

% \begin{Verbatim}[samepage=true]
\usepackage{fancyvrb}

\title{Measuring Metadata Quality}
\author{Péter Király}

% defining \chapterautor
\newcommand{\chapterauthor}[1]{%
  {\parindent0pt\vspace*{0pt}%
  \linespread{1.1}\large\scshape#1%
  \par\nobreak\vspace*{35pt}}
}

% show "Appendix" before appendices
\usepackage{xpatch}
\xapptocmd{\appendix}{%
  \addtocontents{toc}{%
    \RedeclareSectionCommand[
      tocdynnumwidth,
      tocentrynumberformat=\tocappendixnumber
    ]{chapter}%
  }%
}{}{\PatchFailed}
\newcommand\tocappendixnumber[1]{\chapapp~#1}

% JSON highlight
\usepackage{xcolor}
\colorlet{punct}{red!60!black}
\definecolor{background}{HTML}{EEEEEE}
\definecolor{delim}{RGB}{20,105,176}
\colorlet{numb}{magenta!60!black}

% numbers=left,
% numberstyle=\scriptsize,
% stepnumber=1,
% numbersep=8pt,
% backgroundcolor=\color{background},
% frame=lines,
%     literate=
%      *{0}{{{\color{numb}0}}}{1}
%       {1}{{{\color{numb}1}}}{1}
%       {2}{{{\color{numb}2}}}{1}
%       {3}{{{\color{numb}3}}}{1}
%       {4}{{{\color{numb}4}}}{1}
%       {5}{{{\color{numb}5}}}{1}
%       {6}{{{\color{numb}6}}}{1}
%       {7}{{{\color{numb}7}}}{1}
%       {8}{{{\color{numb}8}}}{1}
%       {9}{{{\color{numb}9}}}{1}
%       {:}{{{\color{punct}{:}}}}{1}
%       {,}{{{\color{punct}{,}}}}{1}
%       {\{}{{{\color{delim}{\{}}}}{1}
%       {\}}{{{\color{delim}{\}}}}}{1}
%       {[}{{{\color{delim}{[}}}}{1}
%       {]}{{{\color{delim}{]}}}}{1},

\lstdefinelanguage{json}{
    basicstyle=\normalfont\ttfamily,
    showstringspaces=false,
    breaklines=true,
}
% \input{metadata}

% \usepackage[natbib]{biblatex}
% \addbibresource{bibliography-for-papers.bib}

\begin{document}

% \maketitle
%\renewcommand{\thepage}{\roman{page}}

\frontmatter

\makeatletter

\begin{titlepage}
  %\thispagestyle{empty}
  \begin{center}

  {\sffamily{\bfseries{\Huge\strut\@title}}}
  \vspace{48pt}
  \par
  \par
  Thesis\\
in order to acquire the Doctoral Degree in Philosophy\\
at the Faculty of Humanities of the Georg-August-Universität Göttingen\\
  \vspace{24pt}
  submitted by\\
  {\scshape{\@author}}\\
  from Debrecen\\
  \vspace{12pt}
  {\scshape{Göttingen 2019}}
  \end{center}
  
  \vspace{250pt}
  Thesis supervisors:
  
  {\scshape{Prof. Dr. Gerhard Lauer}}

  {\scshape{Prof. Dr. Ramin Yahyapour}}

  {\scshape{Dr. Marco Büchler}}
%  {\scshape{Prof. Gerhard Lauer,Prof. Ramon Yahyapour,Dr Marco Büchler}}
\end{titlepage}

\makeatother
%\input{titlepage}
\input{abstract_en}

\begin{otherlanguage}{ngerman}
\input{abstract_de}
\end{otherlanguage}

\input{frontmatter}

\mainmatter

\input{chapter01}
\input{chapter02}
\input{chapter03}
\input{chapter04}
\input{chapter05}
\input{chapter06}
\input{chapter07}

\appendix
\input{appendix01}
\input{appendix02}
\input{appendix03}
\input{appendix04}

% \input{backmatter}

\bibliographystyle{acm}
\bibliography{bibliography-for-papers}

% \printbibliography

\end{document}
